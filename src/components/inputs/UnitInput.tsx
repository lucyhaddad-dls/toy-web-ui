...
function SingleUnitDropdown(props: {
  unitType: string;
  onUnitChange: (
    type: "mass_unit" | "length_unit" | "energy_unit",
    option: MassUnits | LengthUnits | EnergyUnits,
  ) => void;
}) {
  const options = defaultSampleUnits.filter(
    (itm) => itm.name == props.unitType,
  )[0].options;
  const [currentOption, setCurrentOption] = useState(
    defaultSampleUnits.filter((itm) => itm.name == props.unitType)[0].value,
  );
        ...
        {options.map((option) => (
          <MenuItem
            ...
            onClick={() => {
              ...
              props.onUnitChange(
                props.unitType as "mass_unit" | "energy_unit" | "length_unit",
                option,
              );
            }}
          >
...
export function UnitSelectComponent() {
  ...
  const onChange = (
    type: "mass_unit" | "energy_unit" | "length_unit",
    option: MassUnits | LengthUnits | EnergyUnits,
  ) => {
  ...
  return (
    <Stack direction="row" spacing={1}>
      {["mass_unit", "energy_unit", "length_unit"].map((val) => (
        <SingleUnitDropdown key={val} unitType={val} onUnitChange={onChange} />
      ))}
    </Stack>
  );
}
