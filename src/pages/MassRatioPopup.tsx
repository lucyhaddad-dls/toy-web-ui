        ...
        <Grid container
         ...
         columnSpacing={{xs:1, sm:2, md:3}}>
                ...
                if (indx == inputCount -1){
                    return (
                        <Grid key={indx} rowSpacing={1} columnSpacing={1}>
                            <MassRatioInput 
                            componentIndex={indx} defaults={formulaInfo[indx]}
                            onChange={onChange}/>
                            <Stack direction="row" spacing={1} 
                            sx = {{ justifyContent: "flex-end" }}>
                            <Fab sx={{bgcolor:"#5f967a", color:"#fefefe"}}
                             size="small" variant="circular"
                            onClick={() => {setInputCount(inputCount + 1); 
                                    onAdd()  }}>
                               <AddIcon/>
                            </Fab>
                            <Fab sx={{bgcolor:"#696969", color:"#fefefe"}} 
                                        size="small" variant="circular"
                            onClick={() => {
                                if (inputCount > 1) {
            ...
            if (indx != inputCount) {
                return (
              <Grid key={indx}>
                <MassRatioInput componentIndex={indx} defaults={formulaInfo[indx]}
                                onChange={onChange}/>
              </Grid>
            );
